...
public actor ViewManager {
    ...
    private func routeInteraction(customId: String, interaction: Interaction, client: DiscordClient) async {
        // Find matching views and dispatch handlers
        for (vid, view) in views {
            // exact match first
            if let handler = view.handlers[customId] {
                Task { await handler(interaction, client) }
                if view.oneShot { await unregister(vid) }
                continue
            }
            // prefix match
            if view.prefixMatch {
                for (key, handler) in view.handlers {
                    if customId.hasPrefix(key) {
                        Task { await handler(interaction, client) }
                        if view.oneShot { await unregister(vid) }
                        break
                    }
